...
def parse_recall_event_concat(save_dir,subjects):
    ...
    with open(os.path.join(save_dir,'event_only_ce_instruct.pkl'),'rb') as f:
        event_only_ce = pickle.load(f)
    with open(os.path.join(save_dir,'recall_event_ce_instruct.pkl'),'rb') as f:
        recall_event_ce_dict = pickle.load(f)
    all_events = []
    all_subjects = []
    all_ce_diff = []
    for event_idx,this_event_ce in enumerate(event_only_ce):
        event_tokens = event_only_stim_dict['input_tokens'][event_idx]
        assert event_tokens.shape[1]-1 ==this_event_ce.shape[0],'tokens should be 1 longer than its CE, cuz BOS token at the start'
        event_start_idx = event_only_stim_dict['event_start_indices'][event_idx]-1
        event_end_idx = event_only_stim_dict['event_end_indices'][event_idx]-1
        this_event_only_ce = this_event_ce[event_start_idx:event_end_idx]
        ...
        for subject_idx in range(len(subjects)):
            ...
            # for debugging
            # if this_event_only_ce.shape != subject_event_ce.shape:
            #     print('event',event_idx,'subject',subject_idx)
            #     print('event post inst',tokenizer.decode(event_tokens[0,event_start_idx+1:event_end_idx+1]))
            #     print('----------------------------------')
            #     print('concat',tokenizer.decode(subject_concat_tokens[0,concat_event_start_idx+1:concat_event_end_idx+1]))
            #     print('event only',this_event_only_ce.shape,'concat',subject_event_ce.shape)
            #     print('entire event tokens',tokenizer.decode(event_tokens[0]))
            #     print('entire concat',tokenizer.decode(subject_concat_tokens[0]))
            assert this_event_only_ce.shape == subject_event_ce.shape,'original and post-concat event CE shape must be the same'
            ce_diff = -torch.sum(subject_event_ce-this_event_only_ce).numpy()
...
def parse_event_recall_concat(save_dir,num_events):
    ...
    with open(os.path.join(save_dir,'recall_only_ce_instruct.pkl'),'rb') as f:
    ...
    with open(os.path.join(save_dir,'event_recall_ce_instruct.pkl'),'rb') as f:
        event_recall_ce_dict = pickle.load(f)
    subjects = recall_only_stim_dict['subject']
    all_events = []
    all_subjects = []
    all_ce_diff = []
    for sub_idx,this_recall_ce in enumerate(recall_only_ce):
        ...
        recall_start_idx = recall_only_stim_dict['recall_start_indices'][sub_idx]-1
        recall_end_idx = recall_only_stim_dict['recall_end_indices'][sub_idx]-1
        sub_recall_only_ce = this_recall_ce[recall_start_idx:recall_end_idx]
